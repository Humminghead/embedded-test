...
// XOSCEN = 0: We provide an EXTERNAL clock on RCLK (generated by TIM2 PWM).
// Per AN332: "Set to 0 for Si4740/41/42/43/44/45/49" (these chips have no crystal osc).
...
async fn main(_s: Spawner) {
    // No LSE / MCO config anymore - the STM32F303VC board has no LSE crystal.
    let p = embassy_stm32::init(Default::default());

    // ================================================================
    // Generate 32.768 kHz reference clock on PA0 (TIM2_CH1) via PWM.
    // ================================================================
    ...
    let mut ch1 = pwm.ch1();
    ch1.enable();
    ch1.set_duty_cycle_fraction(1, 2); // 50% duty cycle

    // Give the Si4743 time to see a stable clock before POWER_UP
    ...
    i2c_config.frequency = Hertz::khz(100); // 100 kHz standard mode
    ...
    // --- POWER_UP (analog audio, FM receive, external clock on RCLK) ---
    match device
    ...
    info!(
        "TUNE 94.10 MHz -> FREQ={} RSSI={} SNR={} AFCRL={} VALID={}",
        freq, rssi, snr, afcrl, valid
    );
